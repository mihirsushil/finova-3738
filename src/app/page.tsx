...
const FEATURES = ['Automatically categorize transactions in real‑time with AI learning', 'Receive personalized cash‑flow forecasts that improve with every use', 'Get started in under 10 minutes — no manual entry or complex setup'];
const PRICING = [{'name': 'Starter', 'price': 'Free', 'features': ['Basic transaction sync', 'AI categorization', 'Monthly insights dashboard']}, {'name': 'Pro', 'price': '$12/mo', 'features': ['Advanced cash‑flow forecasting', 'Multi‑account sync', 'Team sharing & role permissions', 'Priority email support'], 'highlighted': True}, {'name': 'Enterprise', 'price': 'Custom', 'features': ['All Pro features', 'Custom integrations & API access', 'Dedicated account manager', 'SLA & enhanced security']}];
    ...
    <main className="min-h-screen" style={{background: '#0a0a0f'}} >
      ...
      <nav className="fixed top-0 w-full z-50 glass border-b border-white/5">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg" style={{background: 'linear-gradient(135deg, #3b82f6, #f59e0b)'}}></div>
      ...
      <section className="pt-32 pb-24 px-6 text-center relative overflow-hidden">
        <div
          ...
          style={{background: `radial-gradient(ellipse 80% 50% at 50% -20%, #3b82f640, transparent)`}}
        />
        <div className="relative max-w-5xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 justify-center max-w-md mx-auto">
            <input
              ...
              className="flex-1 px-5 py-3 rounded-xl glass text-white placeholder:text-white/30 outline-none focus:border-[#3b82f6] border border-white/10"
              required
            />
            <button
              type="submit"
          ...
          {/* Floating orbs */}
          <div className="absolute -left-20 top-20 w-72 h-72 rounded-full opacity-10 float" style={{background: `radial-gradient(circle, #3b82f6, transparent)`}}></div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {FEATURES.map((feat, i) => (
              <div key={i} className="glass rounded-2xl p-6 hover:border-white/20 transition-all duration-300 group">
                <div className="w-12 h-12 rounded-xl mb-4 flex items-center justify-center" style={{background: '#3b82f620'}}>
            ...
            {[
              ...
              <div key={i} className="glass rounded-2xl p-6">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold" style={{background: 'linear-gradient(135deg, #3b82f6, #10b981)'}}>
                    {t.name[0]}
                  </div>
                  <div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {PRICING.map((tier, i) => (
              <div key={i} className={'glass rounded-2xl p-8 transition-all duration-300 relative ' + (tier.highlighted ? 'glow' : 'hover:border-white/20')}>
                {tier.highlighted && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full text-xs font-bold" style={{background: '#3b82f6'}}>
                    MOST POPULAR
                  </div>
                )}
                <div className="mb-6">
                ...
                <button
                  ...
                  style={{background: tier.highlighted ? 'linear-gradient(135deg, #3b82f6, #10b981)' : 'rgba(255,255,255,0.08)'}}
                >
      ...
      <section className="py-24 px-6">
        <div className="max-w-3xl mx-auto text-center">
          <div className="glass rounded-3xl p-12 md:p-16" style={{background: `linear-gradient(135deg, #3b82f615, #f59e0b10)`}}>
            <h2 className="text-4xl md:text-5xl font-black mb-4">
              Ready to <span className="gradient-text">transform</span> your workflow?
            </h2>
            <p className="text-white/60 text-xl mb-10">Join thousands of teams already using Finova.</p>
            <button
      ...
      <footer className="py-12 px-6 border-t border-white/5">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded" style={{background: 'linear-gradient(135deg, #3b82f6, #f59e0b)'}}></div>
            <span className="font-bold">Finova</span>
          </div>
          <p className="text-white/30 text-sm">
            © {new Date().getFullYear()} Finova. Built with AI. All rights reserved.
          </p>
